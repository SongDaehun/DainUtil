...
Public Class FrmProcessItemExport
    ...
    Public Sub New(ByVal pReportNo As Integer, Optional ByVal pFilepath As String = "")
    ...
    Private Sub FrmProcessItemExport_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        ...
        Try
            ...
            Select Case ReportNo
                Case R_CIPL_LIST
                    Dim i As Integer = 0
                    Dim TempSeqno() As String = Nothing
                    TempSeqno = R_Whare.Split(","c)
                    ProgressBar1.Maximum = TempSeqno.Length

                    Dim Seqno(TempSeqno.Length - 1, 1) As String
                    strSQL = " SELECT * FROM D_CIPL_H WHERE SEQNO IN ( " & R_Whare & " )"
                    cmd = New SqlCommand(strSQL, dbConn)
                    dr = cmd.ExecuteReader
                    While dr.Read
                        If dr("INVOICENO").ToString.ToUpper.Contains("KVPA") Then
                            Seqno(i, 0) = dr("SEQNO")
                            Seqno(i, 1) = dr("INVOICENO").ToString.Substring(0, 4)
                            i = i + 1
                        ElseIf dr("INVOICENO").ToString.ToUpper.Contains("KVPO") Then
                            Seqno(i, 0) = dr("SEQNO")
                            Seqno(i, 1) = dr("INVOICENO").ToString.Substring(0, 4)
                            i = i + 1
                        Else
                            Seqno(i, 0) = dr("SEQNO")
                            Seqno(i, 1) = dr("INVOICENO").ToString.Substring(0, 3)
                            i = i + 1
                        End If
                    ...
                    For i = 0 To TempSeqno.Length - 1
                        If Seqno(i, 1).Contains("KVP") Then
                            CIPL_Report_KVP(Seqno(i, 0))
                        Else
                            CIPL_Report(Seqno(i, 0))
                        End If
    ...
    Private Sub CIPL_Report_KVP(ByVal seqno As String)
        Dim strSQL As String
        Dim cmd As SqlCommand
        Dim dr As SqlDataReader
        ...
        Try
            ...
            strSQL = XmlReadVal(GetQueryFullPath, "root/EXPORT/CI")
            ...
            WorksheetCIPL.Range("A1:Q1").Merge()
            WorksheetCIPL.Range("A1:Q1").Font.Size = 20
            ...
            WorksheetCIPL.Cells(3, 7).Value = "Vendor"
            WorksheetCIPL.Range("G3:G4").Merge()

            WorksheetCIPL.Cells(3, 8).Value = "TOTAL"
            WorksheetCIPL.Cells(4, 8).Value = "Q'TY"

            WorksheetCIPL.Cells(3, 9).Value = "P/Un"
            WorksheetCIPL.Range("I3:I4").Merge()

            WorksheetCIPL.Cells(3, 10).Value = "U/PRICE"
            WorksheetCIPL.Cells(4, 10).Value = "(USD)"

            WorksheetCIPL.Cells(3, 11).Value = "AMOUNT"
            WorksheetCIPL.Cells(4, 11).Value = "(USD)"

            WorksheetCIPL.Cells(3, 12).Value = "N/WEIGHT"
            WorksheetCIPL.Cells(4, 12).Value = "(KGS)"

            WorksheetCIPL.Cells(3, 13).Value = "G/WEIGHT"
            WorksheetCIPL.Cells(4, 13).Value = "(KGS)"

            WorksheetCIPL.Cells(4, 14).Value = "포장개수"
            WorksheetCIPL.Cells(4, 15).Value = "신고세번"
            WorksheetCIPL.Cells(4, 16).Value = "제조사"
            WorksheetCIPL.Cells(4, 17).Value = "협정"

            WorksheetCIPL.Range("A1:Q4").Interior.ColorIndex = 35
            ...
            While dr.Read
                ...
                If SORT & INVOICENUMBER_adjusted <> "" And SORT & INVOICENUMBER_adjusted <> dr("SORT") & dr("INVOICENO_ADJUST") Then
                    WorksheetCIPL.Range("A" & RowNumber.ToString & ":G" & RowNumber.ToString).Merge()
                    ...
                    WorksheetCIPL.Cells(RowNumber, 14).Value = "=SUM(N" & StartRowNumber & ":N" & (RowNumber - 1).ToString & ")"
                ...
                WorksheetCIPL.Cells(RowNumber, 7).Value = dr("VENDER")
                WorksheetCIPL.Cells(RowNumber, 8).Value = dr("PL_QTY")
                WorksheetCIPL.Cells(RowNumber, 9).Value = dr("PL_PUN")
                WorksheetCIPL.Cells(RowNumber, 10).Value = dr("CI_UPRICE_USD")
                WorksheetCIPL.Cells(RowNumber, 11).Value = dr("CI_AMOUNT_USD")
                WorksheetCIPL.Cells(RowNumber, 12).Value = dr("PL_NWEIGHTKGS")
                WorksheetCIPL.Cells(RowNumber, 13).Value = dr("PL_GWEIGHTKGS")
                WorksheetCIPL.Cells(RowNumber, 14).Value = dr("PACKAGEAMOUNT")
                WorksheetCIPL.Cells(RowNumber, 15).Value = dr("HSCODE")
                WorksheetCIPL.Cells(RowNumber, 16).Value = dr("PRODUCT")
                WorksheetCIPL.Cells(RowNumber, 17).Value = dr("CONVENTIONCODE")
            ...
            WorksheetCIPL.Cells.Columns("A:Q").AutoFit()
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeTop).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeLeft).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeRight).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlEdgeBottom).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlInsideHorizontal).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            WorksheetCIPL.Range("A3", "Q" & RowNumber.ToString).Borders(Microsoft.Office.Interop.Excel.XlBordersIndex.xlInsideVertical).LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous
            'G_ReportPath
