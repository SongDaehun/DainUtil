...
Public Class FrmProcessExcelImport
    ...
    End Sub
    Private Sub addlog(ByVal log As String, ByVal type As Integer, Optional ByVal MessageFlag As Boolean = False)
        If txtLog.Text <> "" Then
            Select Case type
                Case 0      '검증
...
#Region "KCP"
    Private Function ImportKCP_CI_Check(ByVal filepath As String) As Boolean
        Dim strSQL As String
        Dim cmd As SqlCommand
        ...
        Try
            ...
            If CISheetCount <> PLSheetCount Then
                addlog("CI시트와 PL시트의 갯수가 틀립니다. CI : " & CISheetCount & "개 / PL : " & "개", 2)
                ...
                If Workbook.Worksheets(j).Name.ToString.Contains("CI") Then
                    ...
                    For k = 0 To 15
                        If WorksheetCI.Cells((k + 23), 1).Value & WorksheetCI.Cells((k + 23), 2).Value <> WorksheetPL.Cells((k + 30), 1).Value & WorksheetPL.Cells((k + 30), 2).Value Then
                            addlog(WorksheetCI.Name.ToString & "의 " & (k + 23).ToString & "열과 " & WorksheetPL.Name.ToString & "의" & (k + 30).ToString & "열의 배열이 상이 합니다.", 2)
    ...
    Private Function ImportKCP_CI(ByVal filepath As String) As Boolean
        ...
        Try
            ...
            For j = 1 To Workbook.Worksheets.Count
                If Workbook.Worksheets(j).Name.ToString.Contains("INVOICE") And Workbook.Worksheets(j).Visible = -1 Then
                    ...
                    ',SHIPPEREXPORTER NVARCHAR(100)
                    strSQL &= " ," & ColumnSet(LTrim(RTrim(WorksheetCI.Cells(3, 1).Value & WorksheetCI.Cells(3, 2).Value & WorksheetCI.Cells(3, 3).Value)))
                    ...
                    ',CONSIGNEEADDRESS NVARCHAR(255)
                    strSQL &= " ,'" & ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(9, 1).Value &
                          WorksheetCI.Cells(9, 2).Value &
                            WorksheetCI.Cells(9, 3).Value))) &
                    ...
                    cmd = New SqlCommand("SELECT top 1 ISNULL(CONTENTSCODE,'') FROM M_CUSTOMCODESET WHERE CODEDIV = 'LOADING PORT'  AND CONTENTS LIKE '%" & WorksheetCI.Cells(14, 3).Value.ToString.ToUpper & "%'", dbConn)
                    dr = cmd.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
                        Loading_Port_Code = dr("CONTENTSCODE")
                    ...
                    'LOADINGPORTNAME NVARCHAR(100)
                    strSQL &= " ,'" & Loading_Port_Code & "'"
                    strSQL &= " ," & ColumnSet(WorksheetCI.Cells(14, 3).Value & WorksheetCI.Cells(14, 3).Value)
                    'DESTINATION NVARCHAR(100)
                    strSQL &= " ," & ColumnSet(WorksheetCI.Cells(16, 3).Value & WorksheetCI.Cells(17, 3).Value)
                    'NOTIFY NVARCHAR(255)
                    strSQL &= " ,'" & ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(14, 4).Value &
                        ...
                        ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(15, 4).Value &
                        ...
                        WorksheetCI.Cells(15, 9).Value &
                        WorksheetCI.Cells(15, 10).Value))) &
                          ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(16, 4).Value &
                        ...
                        WorksheetCI.Cells(16, 9).Value &
                        WorksheetCI.Cells(16, 10).Value))) &
                         ColumnSet(LTrim(RTrim(WorksheetCI.Cells(17, 4).Value &
                    ...
                    'PL_ETA NVARCHAR(8)
                    strSQL &= " ," & ColumnSet(CDate(WorksheetPL.Cells(16, 6).Value).ToString("yyyyMMdd"))
                    ...
                    TotalPallet = WorksheetPL.Cells(18, 8).Value.ToString.ToUpper.Replace(" ", "")
                    'PL_TOTAL_GWEIGHT DECIMAL(14,2)
            ...
            ''2018/03/15
            Dim ProcessCount As Integer = 10
            While XmlReadVal(DirYenFix(G_APPPath) & gsXmlQuery, "root/IMPORT/PROCESS_" & ProcessCount) <> ""
                If XmlReadVal(DirYenFix(G_APPPath) & gsXmlQuery, "root/IMPORT/PROCESS_" & ProcessCount).ToUpper <> "ITEMCHECK" Then
...
#Region "KVPA"
    Private Function ImportKVPA_CI_Check(ByVal filepath As String) As Boolean
        Try
            Return True
        Catch ex As Exception
            Return False
    ...
    ''2018/05/07 수정요청 KVPA_SEA
    Private Function ImportKVPA_CI(ByVal filepath As String) As Boolean
        ...
        Try
            ...
            For j = 1 To Workbook.Worksheets.Count
                If Workbook.Worksheets(j).Name.ToString.Contains("INVOICE") And Workbook.Worksheets(j).Visible = -1 Then
                    ...
                    ',CONSIGNEE NVARCHAR(100)
                    strSQL &= " ," & ColumnSet(WorksheetCI.Cells(7, 1).Value & WorksheetCI.Cells(7, 2).Value & WorksheetCI.Cells(7, 3).Value)
                    ',CONSIGNEEADDRESS NVARCHAR(255)
                    strSQL &= " ,'" & ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(8, 1).Value &
                          WorksheetCI.Cells(8, 2).Value &
                            ...
                            WorksheetCI.Cells(14, 2).Value &
                            WorksheetCI.Cells(14, 3).Value))) &
                            ColumnSetLine(LTrim(RTrim(WorksheetCI.Cells(15, 1).Value &
                            WorksheetCI.Cells(15, 2).Value &
                            WorksheetCI.Cells(15, 3).Value))) & "'"
                    ...
                    'LOADINGPORTNAME NVARCHAR(100)
                    strSQL &= " ,'" & Loading_Port_Code & "'"
                    strSQL &= " ," & ColumnSet(WorksheetCI.Cells(16, 3).Value & WorksheetCI.Cells(16, 3).Value)
                    'DESTINATION NVARCHAR(100)
                    strSQL &= " ," & ColumnSet(WorksheetCI.Cells(18, 3).Value & WorksheetCI.Cells(18, 3).Value)
                    'NOTIFY NVARCHAR(255)
