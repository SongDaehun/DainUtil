...
Module KVPO
    Public Function ImportKVPO_CI_Check(ByVal filepath As String) As Boolean
        ...
        Try
            ...
            If CISheetCount <> PLSheetCount Then
                ...
                FailMessage &= "INVOICE시트와 Packing List시트의 갯수가 틀립니다. INVOICE : " & CISheetCount & "개 / Packing List : " & "개" & PLSheetCount & vbCrLf
            End If
        ...
        End Try
    End Function
    Public Function ImportKVPO_CI(ByVal filepath As String) As Boolean
        ...
        Try
            ...
            For j = 1 To Workbook.Worksheets.Count
                If Workbook.Worksheets(j).Name.ToString.Contains("INVOICE") And Workbook.Worksheets(j).Visible = -1 Then
                    ...
                    strSQL &= " ,'" & LTrim(WorksheetCI.Cells(4, 1).Value & WorksheetCI.Cells(4, 2).Value & WorksheetCI.Cells(4, 3).Value & vbCrLf &
                    ...
                    strSQL &= " ," & ColumnSet(LTrim(WorksheetCI.Cells(8, 1).Value & WorksheetCI.Cells(8, 2).Value & WorksheetCI.Cells(8, 3).Value))
                    ...
                    cmd = New SqlCommand("SELECT top 1 ISNULL(CONTENTSCODE,'') AS CONTENTSCODE FROM M_CUSTOMCODESET WHERE CODEDIV = 'LOADING PORT'  AND UPPER(REPLACE(CONTENTS,' ','')) LIKE '%" & WorksheetCI.Cells(14, 3).Value.ToString.ToUpper.Replace(" ", "") & "%'", dbConn)
                    dr = cmd.ExecuteReader
                    If dr.HasRows Then
                        dr.Read()
